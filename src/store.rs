...
pub fn setup() {
    ...
    conn.execute(
        ...
        sender text, \
        body text, \
        FOREIGN KEY(local_message_id) REFERENCES local_messages(id), \
...
pub fn save_inbound_message(conn: &SqliteConnection, message: &Email) -> Result<i32, SqliteError> {
    conn.execute(
...
pub fn save_local_message(conn: &SqliteConnection, inbound_email: &InboundEmail) -> Result<i32, SqliteError> {
    ...
    conn.execute(
        "INSERT INTO local_messages (recipient, sender, body) VALUES ($1, $2, $3)",
        &[&message.to.to_string(), &message.from.to_string(), &message.body]
    )
...
pub fn save_outbound_message(conn: &SqliteConnection, message: &Email) -> Result<i32, SqliteError> {
    conn.execute(
        "INSERT INTO outbound_messages (recipient, sender, body) VALUES ($1, $2, $3)",
        &[&message.to.to_string(), &message.from.to_string(), &message.body]
    )
}
...
pub fn new_inbound_messages(conn: &SqliteConnection) -> Vec<InboundEmail> {
    let mut statement = conn.prepare(
        "SELECT id, recipient, sender, body from inbound_messages WHERE outbound_message_id IS NULL AND local_message_id IS NULL"
    ).unwrap();
...
//
// TODO: use Result for this
pub fn new_outbound_messages(conn: &SqliteConnection) -> Vec<Email> {
    // TODO: where sent_at = null ?
    let mut statement = conn.prepare(
        "SELECT id, recipient, sender, body from outbound_messages WHERE sent_message_id IS NULL AND local_message_id IS NULL"
    ).unwrap();
    let rows = statement.query(&[]).unwrap();
